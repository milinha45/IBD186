...
public class App 
{
	...
	public static void main( String[] args )
    {
		...
		AutorizacaoRepository autorizacaoRepo = (AutorizacaoRepository) context.getBean("autorizacaoRepository");
		UsuarioRepository usuarioRepo = (UsuarioRepository) context.getBean("usuarioRepository");
		EmpresaRepository empresaRepo = (EmpresaRepository) context.getBean("empresaRepository");
		EstadoRepository estadoRepo = (EstadoRepository) context.getBean("estadoRepository");
		
		SegurancaService seg = (SegurancaService)context.getBean("segurancaService");

		try {
			seg.criarEstado("Rio de Janeiro", "RJ");
		}
		catch(Exception e) {
		...
		try {
			seg.criarEmpresa("Y", "123412364/0001", "RJ");
		}
		catch(Exception e) {
			System.out.println("Rollback realizado!");
			e.printStackTrace();
		...
		Empresa resultEmpresa = seg.queryEmpresaTop1ByNomeContains("Y");
		System.out.println("Nome: " + resultEmpresa.getNome() + " CNPJ: " + resultEmpresa.getCnpj() + resultEmpresa.getEstado().getSigla());
		
		Estado resultEstado = seg.queryEstadoTop1ByNomeContains("Rio de Janeiro");
		System.out.println("Nome: " + resultEstado.getNome() + " Sigla: " + resultEstado.getSigla());
    }    
}
